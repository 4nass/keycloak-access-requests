...
A Keycloak timer handles at most fifty rows per tick. Each row is claimed in a short transaction that commits the `PROCESSING` state, processor token, and lease before any SMTP attempt begins. Delivery and its conditional acknowledgement run in a subsequent, isolated Keycloak transaction and session. Immediately before sending, a worker verifies that its processor token still owns an unexpired lease; the conditional acknowledgement prevents a stale worker from overwriting a newer claim. The committed lease lets other cluster nodes observe that work is in progress and prevents them from claiming it until expiry. The SMTP call is synchronous within this per-delivery transaction, but it no longer holds the original request transaction open. Keep the realm's SMTP connection, read, and write timeouts below the five-minute lease so another node cannot reclaim a delivery while its send is still running. Transient delivery failures are retried with backoff, then retained as `FAILED` after ten attempts for operational follow-up. A missing, disabled, or e-mail-less recipient is recorded as `DISCARDED`.
