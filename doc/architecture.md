...
A Keycloak timer handles at most fifty rows per tick. Every row is leased, delivered, and acknowledged in its own Keycloak transaction and session, so a slow SMTP server or a failed recipient cannot roll back another delivery or keep its database transaction open. The lease and processor token prevent two cluster nodes from handling the same row concurrently. The unique delivery key (`event`, notification type, recipient) makes enqueueing idempotent; transient delivery failures are retried with backoff, then retained as `FAILED` after ten attempts for operational follow-up. A missing, disabled, or e-mail-less recipient is recorded as `DISCARDED`.
